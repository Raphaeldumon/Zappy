...
class aiPlayer
{
  ...
  public:
    aiPlayer(std::uint32_t id, const std::string &team, int x, int y, Orientation orientation)
        : _id(id), _team(team), x(x), y(y), orientation(orientation){};
    ~aiPlayer() = default;
